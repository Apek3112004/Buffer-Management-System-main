...
int main() {
  ...
  try {
    ...
    for (auto &&row : data) {
      ...
      strcpy(employee.fname.data(), row[1].c_str());
      strcpy(employee.lname.data(), row[2].c_str());
    ...
    for (const auto &row : data) {
      ...
      strcpy(company.name.data(), row[1].c_str());
      strcpy(company.slogan.data(), row[2].c_str());
